...
export const handleChatSocket = (socket: Socket) => {
    ...
    // Join a room for the user to handle multiple tabs/devices if needed
    socket.join(`user:${userId}`);

    socket.on("chat:message", async (data: { prompt: string; history: ClientMessage[]; sessionId?: number }) => {
        const { prompt, history, sessionId } = data;
        let activeSessionId = sessionId;

        console.log(`Socket: Message received from user ${userId} for session ${sessionId}`);
        ...
        try {
            ...
            await db.insert(message).values({
                sessionId: activeSessionId!,
                role: "assistant", // "assistant" matches schema enum likely
